				     ...
				     struct irq_data *irq_data)
{
	...
	if (!amd_iommu_guest_ir) {
		data->irte_entry.val = 0;
		modify_irte(irte_info->devid, irte_info->index, data->irte_entry);
	} else {
		data->irte_ga_entry.lo.val = 0;
		data->irte_ga_entry.hi.val = 0;
		modify_irte_ga(irte_info->devid, irte_info->index, &data->irte_ga_entry);
	}
}
...
static char amd_iommu_ir_domain_name[] = "AMD-IOMMU-IR-DOMAIN";
static char amd_iommu_msi_domain_name[] = "AMD-IOMMU-MSI-DOMAIN";
...
static int _set_irte_ga(struct amd_iommu *iommu, unsigned int devid,
...
int amd_iommu_update_vapic_base(u32 vcpu_id, u32 cpu, u32 ga_tag,
				u64 base, bool is_run)
{
	...
	for_each_iommu(iommu) {
		...
		hash_for_each_possible(iommu->ga_hash, ir_data, hnode,
			...
			_set_irte_ga(iommu, ir_data->irq_2_irte.devid,
				     base, cpu, is_run);
		}
...
EXPORT_SYMBOL(amd_iommu_update_vapic_base);
