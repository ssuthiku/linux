...
{
	svm->vmcb->control.avic_vapic_bar = data & 0xFFFFFFFFFF000ULL;
}
...
static void avic_init_vmcb(struct vcpu_svm *svm)
{
	...
	pr_debug("SVM: bpa=%#llx, lpa=%#llx, ppa=%#llx\n", bpa, lpa, ppa);
...
avic_get_phy_ait_entry(struct kvm_vcpu *vcpu, int index)
{
	...
	avic_phy_ait = (struct svm_avic_phy_ait_entry *)
				page_address(vm_data->avic_phy_ait_page);

	return &avic_phy_ait[index];
}

static int avic_clear_phy_ait_entry(struct kvm_vcpu *vcpu)
{
	u8 id;
	...
	entry = avic_get_phy_ait_entry(vcpu, id);
	if (!entry)
		return -EINVAL;
	entry->bk_pg_ptr = 0;
	entry->host_phy_apic_id = 0;
	entry->valid = 0;
	return 0;
}
...
static int avic_init_bk_page(struct kvm_vcpu *vcpu)
{
	...
	pr_debug("SVM: vAPIC bk page: cpu=%u, addr=%#llx, pa=%#llx\n", id, addr,
...
static int cpuid_interception(struct vcpu_svm *svm)
{
	...
	if (avic && (kvm_register_read(&svm->vcpu, VCPU_REGS_RAX) == 1)) {
		u32 ecx = kvm_register_read(&svm->vcpu, VCPU_REGS_RCX);

		/* Do not support X2APIC when enable AVIC */
		ecx &= ~(1 << 21);
		kvm_register_write(&svm->vcpu, VCPU_REGS_RCX, ecx);
	}
	return 1;
}
...
{
	return avic && lapic_in_kernel(vcpu);
}
