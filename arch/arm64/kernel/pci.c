...
int pcibios_root_bridge_prepare(struct pci_host_bridge *bridge)
{
	...
	ctrl = bridge->bus->sysdata;
	ACPI_COMPANION_SET(&bridge->dev, ctrl->companion);
	return 0;
}
...
static int pci_add_mmconfig_region(struct acpi_pci_root_info *ci)
{
	...
	cfg = pci_ecam_lookup(info->controller.segment, info->start_bus);
	if (cfg) {
	...
	cfg = pci_ecam_alloc(info->controller.segment, info->start_bus, info->end_bus,
				 root->mcfg_addr);
	if (!cfg)
		return -ENOMEM;
					   ...
					   int status)
{
	...
	resource_list_for_each_entry_safe(entry, tmp, &ci->resources) {
		 ...
		 * TODO: need move pci_register_io_range() function out of
		 * drivers/of/address.c for both used by DT and ACPI
		 */
